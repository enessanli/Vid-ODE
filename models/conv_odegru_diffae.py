...
class VidODE(nn.Module):
    ...
    def build_model(self):
        ...
        self.adjuster2 = Adjuster(2*base_dim, 16).to(self.device)
        ...
        self.diffae = LitModel(conf)

        # Model ağırlıklarını yükle
        state = torch.load(f'checkpoints/{conf.name}/last.ckpt', map_location='cpu')
        self.diffae.load_state_dict(state['state_dict'], strict=False)
        self.diffae.ema_model.eval()
        self.diffae.ema_model.to(device)
    ...
    def decode(self, x):
        ...
        #print("decoded x:", x.shape)
        return self.adjuster3(x.view(-1, 3, 128, 128))
