...
from models.conv_odegru_diffae import *
...
def get_opt():
    ...
    parser.add_argument('--diff_weight', type=float, default=1.0, help="Difference Loss Weight")
    ...
    parser.add_argument("--image_print_freq", type=int, default=250)
...
def train(opt, netG, loader_objs, device):
    ...
    for epoch in range(opt.epoch):
        ...
        for it in range(n_train_batches):
            ...
            wandb.log({'mse_loss': res_loss-diff_loss, 'diff_loss': diff_loss, 'loss_netG_seq': netG_seq_loss.item(), 'loss_netG_img': netG_img_loss.item(), 'loss_netD_seq': netD_seq_loss.item(), 'loss_netD_img': netD_img_loss.item(), 'total_loss': loss_netG.item(), 'difference_weight': opt.diff_weight})
