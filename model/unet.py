...
class BeatGANsEncoderModel(nn.Module):
    ...
    def forward(self, x, t=None, return_2d_feature=True):
        """
        Apply the model to an input batch.
        ...
        h_2d = h
        print("H 2D shape:", h_2d.shape)
        h = self.out(h)
        print("H shape:", h.shape)
        """
        dosya_adi = f"./latents/frames_h_{self.indexim}.npy"
        if os.path.exists(dosya_adi):
            mevcut_veri = np.load(dosya_adi, allow_pickle=True)
            # Mevcut verinizin boyutu örneğin (m, 512) olsun. Buna yeni veriyi eklemek için:
            guncel_veri = np.concatenate((mevcut_veri, h.cpu().numpy()), axis=0)
        else:
            # Dosya yoksa, yeni veriyi oluşturduğunuz dizi olarak kabul edelim.
            guncel_veri = h.cpu().numpy()
        """ 
        # Güncellenmiş veriyi dosyaya kaydedelim.
        #np.save(dosya_adi, guncel_veri)
        #self.indexim += 1
